...
export function registerAssetMcpTools(server) {
  ...
  reg(
    "arc200_exchange",
    "Read XCHG-1 arc200_exchange() — paired ASA id and sink address for ARC-200 ↔ ASA exchange (simulation). Fails if the contract does not implement the extension.",
    z.object({
      network: NetworkZ,
      appId: AppIdZ,
    }),
    (a) => arc200.arc200Exchange(a.network, a.appId),
    (a, _d) => ({
      network: a.network,
      standard: "arc200",
      method: "arc200_exchange",
      appId: a.appId,
    })
  );
  ...
  reg(
    "arc200_redeem_txn",
    "XCHG-1 arc200_redeem: build unsigned axfer (ASA → app) + redeem app call for base units amount. User must hold the exchange ASA and be opted in.",
    z.object({
      network: NetworkZ,
      appId: AppIdZ,
      sender: AddressZ.describe("Account that sends ASA and signs the group."),
      amount: AmountZ.describe("ASA amount in base units to redeem (must be ≤ axfer amount)."),
    }),
    (a) =>
      arc200
        .arc200RedeemTxn(a.network, a.appId, a.sender, a.amount)
        .then((txns) => ({ transactions: txns })),
    (a, d) => ({
      network: a.network,
      standard: "arc200",
      method: "arc200_redeem_txn",
      appId: a.appId,
      txCount: /** @type {{transactions:string[]}} */ (d).transactions.length,
    })
  );
  ...
  reg(
    "arc200_swap_back_txn",
    "XCHG-1 arc200_swapBack: build unsigned ARC-200 transfer to sink + swapBack app call. Converts ARC-200 back to the configured ASA from the app account.",
    z.object({
      network: NetworkZ,
      appId: AppIdZ,
      sender: AddressZ.describe("Account that sends ARC-200 and signs the group."),
      amount: AmountZ.describe("ARC-200 amount in base units to swap back."),
    }),
    (a) =>
      arc200
        .arc200SwapBackTxn(a.network, a.appId, a.sender, a.amount)
        .then((txns) => ({ transactions: txns })),
    (a, d) => ({
      network: a.network,
      standard: "arc200",
      method: "arc200_swap_back_txn",
      appId: a.appId,
      txCount: /** @type {{transactions:string[]}} */ (d).transactions.length,
    })
  );
